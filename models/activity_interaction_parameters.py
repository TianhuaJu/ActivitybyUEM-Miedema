...
class TernaryMelts:
	...
	def first_derivative_qx (self, i_element: Element, j_element: Element, xi: float = 0) -> float:
		...
		dax = ui * delta_phi * vi * (-vj * (xi * vi + (1 - xi) * vj) - (1 - xi) * vj * (vi - vj)) / (
					(xi * vi + (1 - xi) * vj) ** 2)
		dbx = -uj * delta_phi * vj * (vi * (xi * vi + (1 - xi) * vj) - xi * vi * (vi - vj)) / (
					(xi * vi + (1 - xi) * vj) ** 2)
	...
	def ln_y0 (self, solvent: Element, solutei: Element):
		...
		if self._state == "liquid":
			# (V4.3 修正) Si 和 Ge 的 dh_trans 在 Miedema 模型中通常视为 0
			if solutei.name in ["Si", "Ge"]:
				dhtrans = 0.0
			else:
		...
		chemical_term = 1000 * fik * solutei.v * (1 + solutei.u * (solutei.phi - solvent.phi))
		transition_term = 1000 * dhtrans
		
		lny0 = chemical_term + transition_term
		return lny0 / (Constants.R * self._temperature)
	
	#一阶活度相互作用系数，核心参数
	def activity_interact_coefficient_1st (self, solv: Element, solui: Element, soluj: Element, Tem: float, state: str,
	...
	def roui_jj (self, solv: Element, solui: Element, soluj: Element, Tem: float, state: str, extra_model: extrap_func,
	             extra_model_name="UEM1"):
	...
	def roui_ij (self, solv: Element, solui: Element, soluj: Element, Tem: float, state: str, extra_model: extrap_func,
	             extra_model_name="UEM1"):
	...
	def roui_jk (self, matrix: Element, i: Element, j: Element, k: Element, Tem: float, state: str,
	             extra_model: extrap_func, extra_model_name="UEM1"):
